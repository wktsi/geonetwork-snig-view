...
(function() {
  ...
  goog.provide('gn_search_snig_facets_directive');
  goog.require('gn_utility_service');

  var module = angular.module('gn_search_snig_facets_directive', ['gn_utility_service']);

  
  //Filter for accent insensitive ordering 
  //Reference: http://www.janholinka.net/Blog/Article/8
  module.filter("localeOrderBy", [function () {
  ...
  module.directive('snigFacetDimensionList', [
    ...
    function(gnFacetConfigService, gnLangs) {
      return {
        ...
        scope: {
          dimension: '=snigFacetDimensionList',
          dimension2: '=',
          facetType: '=',
        ...
        link: function(scope, element, attrs) {
          ...
          scope.flgShowFilters = false;
          scope.showFilters = function() {
        	  scope.flgShowFilters = !scope.flgShowFilters;
          }
  ...
  module.directive('snigFacetDimensionCategory', [
    ...
    function(gnFacetConfigService, RecursionHelper, $parse) {
      return {
        ...
        scope: {
          category: '=snigFacetDimensionCategory',
          categoryKey: '=',
          path: '=',
          params: '=',
          facetConfig: '=',
          ...
          return RecursionHelper.compile(element,
              function(scope, element, attrs) {
                ...
                scope.buildLabel = function(category) {
                	...
                	} else if (category['@value'] == 'download') {
                		category['@label'] = 'Descarregamento';
                	}
                ...
                var values = [];
                var undefined_item, other_item;
                for (var i=0; i<scope.category.length; i++) {
                	var cat = scope.category[i];                	                	
                	if (cat['@value'].toLowerCase().endsWith('undefined')) {
	            		undefined_item = {
	            				'@value': cat['@value'],
	            			    '@label': cat['@label'],
	            				'@count': cat['@count']            				
	            			}	            		
                	} else if (cat['@value'].toLowerCase().endsWith('_other')) {
	            		other_item = {
	            				'@value': cat['@value'],
	            			    '@label': cat['@label'],
	            				'@count': cat['@count']            				
                ...
                if (other_item) {
                	values.push(other_item);
                }                
                if (undefined_item) {
                	values.push(undefined_item);
                }
                ...
                scope.getValues= function() {                	                	                	
                	...
                	var query = slugify(scope.filterListValue);
                	                	
                	if (query == "" || query == null) {
                		retVal = values;
                	} else {
                		retVal = values.filter(function (el) {
                			return slugify(el['@label']).contains(query);
                		});
                	}
                ...
                scope.filterList = function () {                	
                	scope.filterListValue = this.filterValue;
                };
